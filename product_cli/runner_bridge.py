...
Calls the UNCHANGED engine `product_kit.mission.run_mission(...)` and serializes the returned
...
per-phase artifacts (discovery.md, strategy.md, …) are produced by the interactive command path;
...
def run(goal: str, project_root: str = ".", steer1: bool = False, steer2: bool = False) -> Path:
    """Headless run: drive the engine, then serialize. Needs openai-agents + a configured provider.

    `steer1=False` (default) auto-proceeds through DC-1 (after strategy, before shape/design).
    `steer2=False` (default) auto-proceeds through DC-2 (after design, before delivery).
    Pass `steer1=True` or `steer2=True` to open the interactive console Direction Check at that gate.
    """
    from product_kit.mission import run_mission, auto_proceed, console_direction_check

    dc1_fn = console_direction_check if steer1 else auto_proceed
    dc2_fn = console_direction_check if steer2 else auto_proceed
    dossier = run_mission(goal, dc1_fn=dc1_fn, dc2_fn=dc2_fn)
